...
// cancelled. Returns the worst exit code (>=8 == error, the old robocopy
// convention); negative if the user cancelled.
// `expectedBytes`/`expectedFiles` must be what the engine will actually copy
// under `policy` (see ExpectedFor). The bar itself spans expected PLUS skipped
// volume: skipped stretches are painted green and advance the bar, but never
// count toward the speed or the ETA.
